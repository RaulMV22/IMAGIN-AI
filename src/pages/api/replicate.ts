  ...
  try {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      if (result.status === "succeeded" && result.output) {
        console.log("Contenido:", typeof result.output);
        console.log("URL:", result.output);
        if (result.output.length > 0 && typeof result.output === "string") {
          return res.status(200).json({ imageUrl: result.output});
        } else {
          return res.status(500).json({ error: "No se obtuvo una URL válida", details: result });
        }
